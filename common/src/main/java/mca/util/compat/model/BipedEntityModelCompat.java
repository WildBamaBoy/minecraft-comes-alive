...
public interface BipedEntityModelCompat {
    ...
    static ModelData getModelData(Dilation dilation, float pivotOffsetY) {
        return getModelData(dilation, dilation, pivotOffsetY);
    }

    static ModelData getModelData(Dilation dilation, Dilation headDilation, float pivotOffsetY) {
        ModelPartData root = new ModelPartData(ModelPartBuilder.create(), dilation);
        root.addChild(EntityModelPartNames.HEAD, ModelPartBuilder.create().uv(0, 0).cuboid(-4, -8, -4, 8, 8, 8, headDilation), ModelTransform.pivot(0, 0 + pivotOffsetY, 0));
        root.addChild(EntityModelPartNames.HAT, ModelPartBuilder.create().uv(32, 0).cuboid(-4, -8, -4, 8, 8, 8, headDilation.add(0.5F)), ModelTransform.pivot(0, 0 + pivotOffsetY, 0));
