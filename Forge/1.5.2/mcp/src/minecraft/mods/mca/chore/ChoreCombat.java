...
public class ChoreCombat extends AbstractChore
{
				...
				if (owner.getNavigator().noPath())
				{
					if (!owner.worldObj.isRemote)
					{
						...
						else
						{
							...
							owner.getNavigator().setPath(owner.getNavigator().getPathToXYZ(sentryPosX, sentryPosY, sentryPosZ), 0.3F);
						}
					}
				}
			}
	...
	private EntityLiving findTarget()
	{
		...
		{
			if (entity instanceof EntityLiving && !(entity instanceof EntityPlayer) && !(entity instanceof AbstractEntity))
			{
				if (closestEntity == null)
				{
					...
					{
						closestEntity = (EntityLiving)entity;
					}
