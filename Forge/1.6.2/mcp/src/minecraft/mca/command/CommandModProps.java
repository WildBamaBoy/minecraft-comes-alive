...
public class CommandModProps extends AbstractCommand
{
	...
	public void processCommand(ICommandSender sender, String[] arguments) 
	{
		if (arguments[0].trim().equalsIgnoreCase("set") || arguments[0].trim().equalsIgnoreCase("get") || arguments[0].trim().equalsIgnoreCase("list"))
		{
			...
			catch (Exception e)
			{
				...
				sender.sendChatToPlayer(new ChatMessageComponent().func_111072_b(Constants.COLOR_RED + "An unknown exception has occured."));
			}
		}
		...
		if (arguments.length == 3)
		{
			...
			for (final Field field : ModPropertiesList.class.getDeclaredFields())
			{
				if (field.getName().equalsIgnoreCase(stringToFind))
				{
					...
					sender.sendChatToPlayer(new ChatMessageComponent().func_111072_b(Constants.COLOR_YELLOW + "Value of " + arguments[1] + " set to: " + arguments[2]));
					return;
				}
			}

			sender.sendChatToPlayer(new ChatMessageComponent().func_111072_b(Constants.COLOR_RED + "Mod property not found: " + arguments[1]));
	...
	private void getModProperty(ICommandSender sender, String[] arguments) throws IllegalArgumentException, IllegalAccessException
	{
		if (arguments.length == 2)
		{
			...
			for (final Field field : ModPropertiesList.class.getDeclaredFields())
			{
				if (field.getName().equalsIgnoreCase(stringToFind))
				{
					sender.sendChatToPlayer(new ChatMessageComponent().func_111072_b(Constants.COLOR_YELLOW + "Value of " + arguments[1] + " equals: " + field.get(MCA.getInstance().modPropertiesManager.modProperties).toString()));
					return;
				}
			}
	...
	private void listModProperties(ICommandSender sender, String[] arguments) throws IllegalArgumentException, IllegalAccessException
	{
		if (arguments.length == 1)
		{
			for (final Field field : ModPropertiesList.class.getDeclaredFields())
			{
				sender.sendChatToPlayer(new ChatMessageComponent().func_111072_b(Constants.COLOR_YELLOW + "<" + field.getName() + "> = " + field.get(MCA.getInstance().modPropertiesManager.modProperties).toString()));
			}
		}
