				...
				if (isInAnvilGiftMode)
				{
					if (itemStack.getItem().itemID == itemIdRequiredForSale)
					{
						if (itemStack.stackSize >= amountRequiredForSale)
						{
							...
							PacketDispatcher.sendPacketToServer(PacketHandler.createAddItemPacket(Block.anvil.blockID, player.entityId));
							PacketDispatcher.sendPacketToServer(PacketHandler.createFieldValuePacket(entityId, "isInAnvilGiftMode", false));
							PacketDispatcher.sendPacketToServer(PacketHandler.createFieldValuePacket(entityId, "hasGivenAnvil", true));
						}
	...
	private void updateDivorce()
	{
		...
		if (shouldDivorce)
		{
			if (isSpouse)
			{
				...
				WorldPropertiesManager worldPropertiesManager = MCA.getInstance().playerWorldManagerMap.get(lastInteractingPlayer);
				...
				if (getDistanceToEntity(worldObj.getPlayerEntityByName(lastInteractingPlayer)) < 10)
				{
					if (!worldObj.isRemote)
					{
						...
						{
							this.dropItem(MCA.getInstance().itemWeddingRing.itemID, 1);
						}
