 ...
 */
public abstract class EntityBase extends EntitySerializable implements Serializable
{
	//Primitive types
	public String name = "";
		...
		try
		{
			for (Field f : this.getClass().getFields())
			{
				if (EntityChild.class.isAssignableFrom(f.getDeclaringClass()) || EntityPlayerChild.class.isAssignableFrom(f.getDeclaringClass()) ||
						...
						EntityBase.class.isAssignableFrom(f.getDeclaringClass()))
				{
					try
					{
						fieldName = f.getName();
	...
	protected void damageEntity(DamageSource damageSource, float damageAmount)
	{
		if (!currentChore.equals("Hunting"))
		{
			if (name.equals("Chell") && EntityBase.getBooleanWithProbability(50) && !damageSource.isUnblockable())
			{
				//Skip!
				return;
			}
	 ...
	 */
	protected int getHearts(EntityPlayer player)
	{
		int hearts = 0;
	 ...
	 */
	protected void modifyHearts(EntityPlayer player, int amount)
	{
		if (playerMemoryMap.containsKey(player.username))
		{
	 ...
	 */
	public static void faceCoordinates(EntityBase entity, double posX, double posY, double posZ)
	{
