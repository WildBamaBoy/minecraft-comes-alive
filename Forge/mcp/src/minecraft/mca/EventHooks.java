...
public class EventHooks 
{
	...
	public void itemTossedEventHandler(ItemTossEvent event)
	{
		if (event.entityItem.getEntityItem().getItem() instanceof ItemBaby)
		{
			...
			{
				PacketDispatcher.sendPacketToPlayer(PacketCreator.createSayLocalizedPacket(event.player, null, "notify.player.droppedbaby", false, null, null), (Player)event.player);
				event.player.inventory.addItemStackToInventory(event.entityItem.getEntityItem());

				if (event.entity.worldObj.isRemote)
				{
					PacketDispatcher.sendPacketToServer(PacketCreator.createAddItemPacket(event.entityItem.getEntityItem().itemID, event.player.entityId));
				}
	...
	public void entityJoinedWorldEventHandler(EntityJoinWorldEvent event)
	{
		...
		if (!event.world.isRemote)
		{
			...
			if (event.entity instanceof EntityVillager && !(event.entity instanceof EntitySerializable))
			{
				...
				if (villager.getProfession() < 6 && villager.getProfession() > -1)
				{
					...
					case 4: 
						if (EntityBase.getBooleanWithProbability(50))
						{
