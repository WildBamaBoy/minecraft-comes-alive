...
public class RenderHuman extends RenderLiving
{
	...
	{
		renderHuman((EntityBase)entity, posX, posY, posZ, rotationYaw, rotationPitch);
	}
	...
	{
		renderHuman((EntityBase)entityLiving, posX, posY, posZ, rotationYaw, rotationPitch);
	}

	@Override
	protected int shouldRenderPass(EntityLivingBase EntityLivingBase, int armorId, float partialTickTime)
	{
		return setArmorModel((EntityBase)EntityLivingBase, armorId, partialTickTime);
	}
	...
	{
		//Scale the entity.
		if (((EntityBase)EntityLivingBase).gender.equals("Female"))
		{
	...
	{
		if (!((EntityBase)EntityLivingBase).currentChore.equals("Hunting"))
		{
			super.renderEquippedItems(EntityLivingBase, partialTickTime);

			try
			{
				EntityBase entity = (EntityBase)EntityLivingBase;
	...
	protected void passSpecialRender(EntityLivingBase EntityLivingBase, double posX, double posY, double posZ)
	{
		if (!((EntityBase)EntityLivingBase).currentChore.equals("Hunting"))
		{
			renderLabels((EntityBase)EntityLivingBase, posX, posY, posZ);
		}
	}
	...
	{
		if (!((EntityBase)par1EntityLivingBase).currentChore.equals("Hunting"))
		{
	 ...
	 */
	private int setArmorModel(EntityBase entity, int armorId, float partialTickTime)
	{
		try
		{
	...
	{
		if (((EntityBase)entity).hasBeenExecuted)
		{
		...
		if (!((EntityBase)entity).currentChore.equals("Hunting"))
		{
	 ...
	 * @param 	posZ	The entity's z position.
	 */
	private void renderLabels(EntityBase entity, double posX, double posY, double posZ)
	{
		...
		{	
			...
			else
			{
				...
				EntityBase serverEntity = (EntityBase)DimensionManager.getWorld(entity.worldObj.provider.dimensionId).getEntityByID(entity.entityId);
	 ...
	 * @param 	posZ	The entity's z position.
	 */
	private void rotateHuman(EntityBase entity, float posX, float posY, float posZ)
	{
	 ...
	 * @param 	posZ	The entity's z position.
	 */
	private void renderHumanSleep(EntityBase entity, double posX, double posY, double posZ)
	{
	 ...
	 */
	private void renderLabel(EntityBase entityBase, double posX, double posY, double posZ, String labelText)
	{
