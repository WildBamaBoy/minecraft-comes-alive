...
public class RenderHuman extends RenderBiped
{
	...
	@Override
	protected int shouldRenderPass(EntityLiving EntityLiving, int armorId, float partialTickTime)
	{
		return setArmorModel((AbstractEntity)EntityLiving, armorId, partialTickTime);
	}
	...
	{
		//Scale the entity.
		if (((AbstractEntity)EntityLiving).gender.equals("Female"))
		{
	...
	{
		if (!((AbstractEntity)EntityLiving).currentChore.equals("Hunting"))
		{
			super.renderEquippedItems(EntityLiving, partialTickTime);

			try
			{
				AbstractEntity entity = (AbstractEntity)EntityLiving;
	...
	@Override
	protected void passSpecialRender(EntityLiving EntityLiving, double posX, double posY, double posZ)
	{
		if (!((AbstractEntity)EntityLiving).currentChore.equals("Hunting"))
		{
			renderLabels((AbstractEntity)EntityLiving, posX, posY, posZ);
		}
	}

	@Override
	protected void rotateCorpse(EntityLiving EntityLiving, float posX, float posY, float posZ)
	{
		if (!((AbstractEntity)EntityLiving).currentChore.equals("Hunting"))
		{
			super.rotateCorpse(EntityLiving, posX, posY, posZ);
		}
	}

	@Override
	protected void renderLivingAt(EntityLiving EntityLiving, double posX, double posY, double posZ)
	{
		if (!((AbstractEntity)EntityLiving).currentChore.equals("Hunting"))
		{
			super.renderLivingAt(EntityLiving, posX, posY, posZ);
		}
	}
		...
		try
		{
			...
			if (itemStack != null)
			{
				...
				if (item instanceof ItemArmor)
				{
					...
					loadTexture((new StringBuilder()).append("/armor/").append(bipedArmorFilenamePrefix[itemArmor.renderIndex]).append("_").append(armorId != 2 ? 1 : 2).append(".png").toString());
	 ...
	 */
	private void renderHuman(EntityLiving entity, double posX, double posY, double posZ, float rotationYaw, float rotationPitch)
	{
