...
import mods.mca.chore.AbstractChore;
import mods.mca.chore.ChoreCombat;
...
import mods.mca.enums.EnumMoodChangeContext;
import mods.mca.enums.EnumRelation;
import mods.mca.enums.EnumTrait;
import mods.mca.inventory.Inventory;
...
public abstract class AbstractEntity extends AbstractSerializableEntity implements Serializable
{
	...
	public transient EntityLiving target = null;
	...
	@Override
	protected void damageEntity(DamageSource damageSource, int damageAmount)
	{
		if (!currentChore.equals("Hunting"))
		{
			...
			int unabsorbedDamage = ISpecialArmor.ArmorProperties.ApplyArmor(this, inventory.armorItems, damageSource, damageAmount);
			...
			{
				if (target != null && (damageSource.getSourceOfDamage() instanceof EntityLiving))
				{
					target = (EntityLiving)damageSource.getSourceOfDamage();
					PacketDispatcher.sendPacketToAllPlayers(PacketHelper.createSetTargetPacket(entityId, target.entityId));
				}
			}
	...
	private void updateRetaliation()
	{
		if (isRetaliating && target != null)
		{
			if (target instanceof EntityPlayer)
			{
				...
				getNavigator().tryMoveToEntityLiving(player, 0.3F);
	 ...
	 */
	public static void faceCoordinates(EntityLiving entity, double posX, double posY, double posZ, int rotationPitch)
	{
		double deltaX = posX - entity.posX;
		double deltaY = entity.posY - posY;
		double deltaZ = posZ - entity.posZ;
