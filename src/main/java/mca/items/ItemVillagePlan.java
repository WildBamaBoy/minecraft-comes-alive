...
public class ItemVillagePlan extends ItemMapBase {
    protected ItemVillagePlan()
    {
    ...
    {
        if (worldIn.provider.getDimension() == data.dimension && viewer instanceof CPlayer)
        {
            int i = 1 << data.scale;
            int j = data.xCenter;
            int k = data.zCenter;
            ...
            MapData.MapInfo mapdata$mapinfo = data.getMapInfo((CPlayer)viewer);
                ...
                if ((k1 & 15) == (mapdata$mapinfo.step & 15) || flag)
                {
                    ...
                    for (int l1 = i1 - j1 - 1; l1 < i1 + j1; ++l1)
                    {
                        if (k1 >= 0 && l1 >= -1 && k1 < 128 && l1 < 128)
                        {
                            ...
                            Chunk chunk = worldIn.getChunkFromBlockCoords(new CPos(k2, 0, l2));
                            ...
                            if (!chunk.isEmpty())
                            {
                                ...
                                if (worldIn.provider.isNether())
                                {
                                    ...
                                    if ((l3 >> 20 & 1) == 0)
                                    {
                                        multiset.add(Blocks.DIRT.getDefaultState().withProperty(BlockDirt.VARIANT, BlockDirt.DirtType.DIRT).getMapColor(worldIn, CPos.ORIGIN), 10);
                                    }
                                    else
                                    {
                                        multiset.add(Blocks.STONE.getDefaultState().withProperty(BlockStone.VARIANT, BlockStone.EnumType.STONE).getMapColor(worldIn, CPos.ORIGIN), 100);
                                    }
                                ...
                                {
                                    CPos.MutableCPos blockpos$mutableblockpos = new CPos.MutableCPos();
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected)
    {
        if (!worldIn.isRemote)
        {
            ...
            if (entityIn instanceof CPlayer)
            {
                CPlayer entityplayer = (CPlayer)entityIn;
                mapdata.updateVisiblePlayers(entityplayer, stack);
            }

            if (isSelected || entityIn instanceof CPlayer && ((CPlayer)entityIn).getHeldItemOffhand() == stack)
            {
    ...
    @Nullable
    public Packet<?> createMapDataPacket(ItemStack stack, World worldIn, CPlayer player)
    {
        return this.getMapData(stack, worldIn).getMapPacket(stack, worldIn, player);
    }

    public void onCreated(ItemStack stack, World worldIn, CPlayer playerIn)
    {
        CNBT nbttagcompound = stack.getTagCompound();
    ...
    {
        CNBT nbttagcompound = p_190907_0_.getSubCompound("display");
