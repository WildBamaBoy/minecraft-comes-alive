...
public class TextRendererCompat {
    ...
    private static class OutlinedDrawer implements OrderedText {
        ...
        public boolean accept(CharacterVisitor visitor) {
            TextRenderer.Drawer drawer = (TextRenderer.Drawer)visitor;

            float originalX = drawer.x;
            float originalY = drawer.y;
                ...
                for(int n = -1; n <= 1; ++n) { // top|middle|bottom
                    if (m != 0 || n != 0) { // skip the middle one
                        ...
                        text.accept((lx, style, mx) -> {
                            ...
                            FontStorage fontStorage = renderer.getFontStorage(style.getFont());
                            Glyph glyph = fontStorage.getGlyph(mx);
                            drawer.x = fs[0] + fs2[0] * glyph.getShadowOffset();
                            drawer.y = y + fs2[1] * glyph.getShadowOffset();
                            fs[0] += glyph.getAdvance(bl);
                            return drawer.accept(lx, style.withColor(tc), mx);
                        });
                    }
                }
            }
