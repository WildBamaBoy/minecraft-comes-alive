...
public class EntityGrimReaper extends EntityMob implements IBossDisplayData
{
	...
	protected void attackEntity(Entity entity, float damage) 
	{
		...
		if (RadixMath.getDistanceToEntity(entityToAttack, this) <= 1.8D)
		{
			...
			{
				((EntityLivingBase)entity).addPotionEffect(new PotionEffect(Potion.wither.id, this.worldObj.difficultySetting.getDifficultyId() * 20, 1));
			}
				...
				if (entityToAttack instanceof EntityPlayer)
				{
					...
					if (player.isBlocking())
					{
						...
						if (!worldObj.isRemote && RadixLogic.getBooleanWithProbability(20))
						{
							...
							player.addPotionEffect(new PotionEffect(Potion.blindness.id, this.worldObj.difficultySetting.getDifficultyId() * (Time.SECOND * 2), 1));
						}
					}
	...
	{
		return worldObj.getClosestVulnerablePlayerToEntity(this, 48.0D);
	}
	...
	public void onUpdate() 
	{
		...
		{
			floatingTicks += 0.1F;
			Utilities.spawnParticlesAroundEntityC(Particle.VOID, this, 1);
			Utilities.spawnParticlesAroundEntityC(Particle.MAGIC_CRITICAL_HIT, this, 1);
		}
		...
		if (!worldObj.isRemote)
		{
			...
			for (Point3D point : grassBlocks)
			{
				...
				{
					BlockHelper.setBlock(worldObj, point.iPosX, point.iPosY + 1, point.iPosZ, Blocks.air, 0);
				}
			}
		}
	}
	...
	private void teleportTo(double x, double y, double z)
	{
		if (!worldObj.isRemote)
		{
			Utilities.spawnParticlesAroundEntityS(Particle.PORTAL, this, 16);
