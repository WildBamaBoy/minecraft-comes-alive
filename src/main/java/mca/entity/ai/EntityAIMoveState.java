...
public class EntityAIMoveState extends EntityAIBase {
    ...
    public void updateTask() {
        ...
        Player playerToFollow = villager.world.getPlayerEntityByUUID(villager.playerToFollowUUID);

        switch (EnumMoveState.byId(villager.get(EntityVillagerMCA.MOVE_STATE))) {
            case FOLLOW:
                double distance = playerToFollow != null ? villager.getDistance(playerToFollow.getPlayer()) : -1.0D;
                if (playerToFollow == null) {
                    villager.set(EntityVillagerMCA.MOVE_STATE, EnumMoveState.MOVE.getId());
                    break;
                } else if (distance >= 3.0D && distance <= 10.0D) {
                    nav.setPath(nav.getPathToEntityLiving(playerToFollow.getPlayer()), villager.isRiding() ? 1.7D : 0.8D);
                } else if (distance > 10.0D) {
                    villager.attemptTeleport(playerToFollow.getPosX(), playerToFollow.getPosY(), playerToFollow.getPosZ());
