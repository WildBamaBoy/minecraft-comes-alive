...
public class GrimReaperMelee extends Goal {
    ...
    public boolean canUse() {
        ...
        return entityToAttack != null && reaper.distanceTo(entityToAttack) <= 12.0D && reaper.tickCount > lastAttack + COOLDOWN && reaper.getAttackState() != EnumReaperAttackState.REST;
    }
    ...
    public void tick() {
        ...
        if (blockDuration > 0) {
            ...
            if (reaper.distanceTo(entityToAttack) <= 2.0D) {
            ...
            //attack
            if (reaper.distanceTo(entityToAttack) <= 1.0D) {
